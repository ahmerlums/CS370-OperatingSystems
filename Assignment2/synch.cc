...
void Condition::Wait(Lock* conditionLock) { 
    ...
    Semaphore * waiter=new Semaphore("condition",0);
...
void static wakeUp(int thread){

    IntStatus old = interrupt->SetLevel(IntOff); // set interupt
    scheduler->ReadyToRun((Thread*)thread); // add to ready queue
    interrupt->SetLevel(old);
}

void Alarm::goToSleepFor(int howLong){
    interrupt->Schedule(wakeUp,(int)currentThread,howLong,TimerInt);
    currentThread->Sleep();
}